...
async function main() {
  ...
  const neededCollections = ['mekhalas', 'parishes', 'events', 'candidates'];
  ...
  const Parish = mongoose.model('Parish', new mongoose.Schema({
    name: { type: String, unique: true },
    mekhala: String,
    createdAt: { type: Date, default: Date.now },
  }));
  ...
  const parishCount = await Parish.countDocuments();
  if (parishCount === 0) {
    console.log('Seeding initial Parishes...');
    await Parish.insertMany([
